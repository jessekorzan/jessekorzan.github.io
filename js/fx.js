    ...
    jk.parallax = {
    	...
    	animate: function () {
        	jk.config.$backgrounds.each(function(index) {
        		var fg = $(this).find(".container"),
					bg = $(this).find(".bg"),
					y = (($(this).offset().top - jk.config.winY) * jk.config.factor + (index+1)),
					top = $(this).offset().top,
    			...
    			if ($(bg).length) {
    				$(bg).css({"-webkit-transform": "translate3d(0," + (-y) + "px, 0)"});
					$(bg).css({"-moz-transform": "translate3d(0," + (-y) + "px, 0)"});
					$(bg).css({"-ms-transform": "translate3d(0," + (-y) + "px, 0)"});
				...
				if ($(fg).length && !$(this).hasClass("on")) {
					...
					var o = 1 - (Math.abs(y / $(this).innerHeight())*3),
						oo = o / 1;
    				...
    				$(fg).css({"-webkit-transform": "translate3d(0," + y + "px, 0)"});
					$(fg).css({"-moz-transform": "translate3d(0," + y + "px, 0)"});
					$(fg).css({"-ms-transform": "translate3d(0," + y + "px, 0)"});
					$(fg).css({"transform": "translate3d(0," + y + "px, 0)"});
				}
			});
        },
    	loop : function () {
